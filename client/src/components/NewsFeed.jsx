...
function formatExactReceived(dateStr) {
  if (!dateStr) return 'Just now';
  ...
  return `${istStr} IST (${utcHours}:${utcMins}:${utcSecs} UTC)`;
}
...
function NewsItem({ item, isNew }) {
  ...
  const displaySummary = item.summary && item.summary.trim().length > 0
    ? item.summary
    : `Direct financial wire update from ${item.source || 'Financial Wire'}. This event carries direct implications for US interest rate projections, liquidity conditions, and safe-haven spot gold (XAU/USD) flows.`;

  const receivedLabel = timeAgo(item.processedAt || item.publishedAt);
  const exactTimeLabel = formatExactReceived(item.processedAt || item.publishedAt);
    ...
    >
      ...
      <div className="news-item-header">
        ...
        <div className="news-badges">
          ...
          {/* Quick External Link Button (opens article in 1 click without needing to expand) */}
          {item.link && (
            <button
              type="button"
              className="news-header-link-btn"
              title={`Open article on ${item.source || 'publisher'}`}
          ...
          <button
            ...
            onClick={handleToggleExpand}
            style={{
              background: expanded ? 'rgba(245, 166, 35, 0.2)' : 'rgba(255,255,255,0.06)',
      ...
      {/* ─── Collapsed Quick Info: Reasoning & Time Received ───────────── */}
      {!expanded && (
        <>
          {item.reasoning && (
            <div className="news-item-reasoning" style={{ opacity: 0.88, margin: '6px 0' }}>
              💡 {item.reasoning}
            </div>
          )}

          <div className="news-item-footer" style={{ marginTop: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span className="news-time" style={{ color: 'var(--gold-muted)', fontWeight: 600, fontFamily: 'var(--font-mono)', fontSize: '0.64rem' }}>
              ⏱️ {receivedLabel}
            </span>
          </div>
        </>
      ...
      {expanded && (
        ...
        >
          {/* Timestamp Banner: When News Was Received */}
          <div style={{
            ...
            <div style={{ color: 'var(--gold-bright)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>⏱️</span>
              <span><strong>Received:</strong> {exactTimeLabel}</span>
            </div>
          ...
          {/* Detailed Reasoning & Trading Bias Box */}
          <div style={{
            background: 'rgba(0, 0, 0, 0.45)',
            borderLeft: `4px solid ${item.bias === 'BULLISH' ? 'var(--bull-primary)' : item.bias === 'BEARISH' ? 'var(--bear-primary)' : 'var(--neutral-primary)'}`,
            padding: '9px 12px',
            ...
            <div style={{
              ...
              📊 Quant & AI Trading Signal Rationale
            </div>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-primary)', lineHeight: 1.5 }}>
              {item.reasoning || 'No specific high-conviction catalyst detected. Maintain defensive positioning and watch key session levels.'}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div>
              <span style={{ color: 'var(--gold-muted)' }}>Publisher:</span> {item.source || 'Financial Wire'}
            </div>
            {item.relevanceScore !== undefined && (
              <div>
                <span style={{ color: 'var(--gold-muted)' }}>XAU Relevance:</span> {item.relevanceScore}/10
              </div>
            )}
            <div>
              <span style={{ color: 'var(--gold-muted)' }}>Scoring Engine:</span> {item.model || 'Scorer'}
            </div>
          </div>

          {/* Action Row: High-Visibility Direct Redirect Button */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
            ...
            {item.link ? (
              <a
                ...
                style={{
                  ...
                  boxShadow: '0 3px 14px rgba(245, 166, 35, 0.2)',
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                }}
              >
                <span>🌐</span> Read Full Story on {item.source || 'Publisher'} ↗
              </a>
            ) : (
              <span style={{
                fontSize: '0.68rem',
                ...
                Direct article URL not provided by publisher feed
              </span>
            )}
